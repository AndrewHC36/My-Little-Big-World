...
class Character:
    ...
    def collision(self, blockt):
        ...
        for i in range(-BLOCK_SZ*COLLISION_SZ, BLOCK_SZ*COLLISION_SZ+1, BLOCK_SZ):
            for j in range(-BLOCK_SZ*COLLISION_SZ, BLOCK_SZ*COLLISION_SZ+1, BLOCK_SZ):
                if blockt[x//BLOCK_SZ+i//BLOCK_SZ-TERRAIN_VIEWBOX[0]][y//BLOCK_SZ+j//BLOCK_SZ-TERRAIN_VIEWBOX[1]+1] != BLK["air"]:  # BOTTOM
                    if (y < j+B+self.ty < Y+BT_P) and (i+a+self.tx < x < i+A+self.tx or i+a+self.tx < X < i+A+self.tx): COL.append(SOUTH)
                if blockt[x//BLOCK_SZ+i//BLOCK_SZ-TERRAIN_VIEWBOX[0]][y//BLOCK_SZ+j//BLOCK_SZ-TERRAIN_VIEWBOX[1]] != BLK["air"]:  # TOP
                    if (Y > j+b+self.ty > y+TP_P) and (i+a+self.tx < x < i+A+self.tx or i+a+self.tx < X < i+A+self.tx): COL.append(NORTH)
                if blockt[x//BLOCK_SZ+i//BLOCK_SZ-TERRAIN_VIEWBOX[0]-1][y//BLOCK_SZ+j//BLOCK_SZ-TERRAIN_VIEWBOX[1]] != BLK["air"]:  # LEFT
                    if (X > i+a+self.tx > x+LT_P) and (j+b+self.ty < y < j+B+self.ty or j+b+self.ty < Y < j+B+self.ty): COL.append(WEST)
                if blockt[x//BLOCK_SZ+i//BLOCK_SZ-TERRAIN_VIEWBOX[0]+1][y//BLOCK_SZ+j//BLOCK_SZ-TERRAIN_VIEWBOX[1]] != BLK["air"]:  # RIGHT
    ...
    def edit(self, loc, key, chosenBlock):
        ...
        BXI, BYI = (loc[0]-self.tx)//BLOCK_SZ-1, (loc[1]-self.ty)//BLOCK_SZ-1  # Block's [x/y] index of the block terrain
        BXL, BYL = (loc[0]-self.tx)//BLOCK_SZ-1*BLOCK_SZ, (loc[1]-self.ty)//BLOCK_SZ-1*BLOCK_SZ  # Block [x/y] location
        ...
        return BXI, BYI, BLOCK, True  # Coordinates of BLOCK TERRAIN/Break or Place - POS, BLOCK_ID
